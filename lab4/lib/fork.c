...
pgfault(struct UTrapframe *utf)
{
	...
	if((err & FEC_WR) == 0 || (uvpt[(unsigned)addr >> 12] & PTE_P) == 0 || (uvpt[(unsigned)addr >> 12] & PTE_COW) == 0 || (uvpd[PDX(addr)] & PTE_P) == 0){
		panic("Faulting access not valid!\n");
	}
...
duppage(envid_t envid, unsigned pn)
{
	...
	// LAB 4: Your code here.
	unsigned addr = pn * PGSIZE;
	
	int perm;
	if((uvpt[addr >> 12] & (PTE_W | PTE_COW)) != 0){
...
fork(void)
{
	...
	for(addr = 0; addr < UXSTACKTOP - PGSIZE; addr += PGSIZE){
		if((uvpd[PDX(addr)] & PTE_P) != 0 && (uvpt[addr >> 12] & PTE_P) != 0 && (uvpt[addr >> 12] & PTE_U) != 0)
