...
page_init(void)
{
	...
	size_t EndOfPages = (size_t)envs + (sizeof(struct Env) * NENV);
	cprintf("%x\n",EndOfPages);
	...
	cprintf("npages:%d\n",npages);
	for (i = 1; i < npages; i++) {
		if(i >= (unsigned int)MPENTRY_PADDR / PGSIZE && i < IOHole) continue;
		pages[i].pp_ref = 0;
...
page_alloc(int alloc_flags)
{
	...
	if(alloc_flags & ALLOC_ZERO){
		uintptr_t va = (uintptr_t)page2kva(free_addr);
		memset((char*)va, 0, PGSIZE);
	}
...
user_mem_check(struct Env *env, const void *va, size_t len, int perm)
{
	...
	for(i = 0; i < num_pages; i++){
		...
		if(curr_va > ULIM){
			if(curr_va >= (size_t)va)
				user_mem_check_addr = curr_va;
			else
				user_mem_check_addr = (size_t)va;
			return -E_FAULT;
		}
		pte_t* content = pgdir_walk(env->env_pgdir, (const void*)curr_va, 0);
		if(content == NULL || ((*content) & (perm|PTE_P)) != (perm | PTE_P)){
			if(curr_va >= (size_t)va)
				user_mem_check_addr = curr_va;
			else
				user_mem_check_addr = (size_t)va;
